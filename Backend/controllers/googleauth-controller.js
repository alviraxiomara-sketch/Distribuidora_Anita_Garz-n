...
import { obtenerUsuarioPorCorreo, crearUsuarioGoogle, actualizarUsuario } from "../models/user-model.js";
...
export const autenticarConGoogle = async (req, res) => {
    try {
        ...
        // Google devuelve 'email', lo mapeamos a 'correo'
        const { sub: googleId, email: correo, name: nombre, picture: avatar } = payload;

        // 2. Comprobar si ya existe en Supabase
        const { data: usuarioExistente } = await obtenerUsuarioPorCorreo(correo);
        ...
        if (usuarioExistente) {
            ...
            if (Object.keys(camposActualizar).length > 0) {
                await actualizarUsuario(usuarioExistente.id_usuario, camposActualizar);
            }
        } else {
        ...
        const token = jwt.sign(
            { id_usuario: usuarioFinal.id_usuario, rol: usuarioFinal.rol },
